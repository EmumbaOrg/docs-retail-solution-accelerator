...
2. **Input your choice for the Azure Container Apps deployment**: Enter `no` to skip Azure Container Apps deployment and press enter.
   ...
   ```bash title=""
    ...
    ```
   - Selecting `no` will deploy only the azure_ai and the database server in the resource-group --- the frontend, backend and arize will not be deployed.
   - Selecting `yes` here will deploy everything in the resource-group.
